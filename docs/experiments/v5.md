...
ESC-50 fold 1~3은 학습, fold 4는 검증, fold 5는 최종 테스트에 사용합니다. 현장 `data/dataset/normal/*.wav`는 라벨 0, `data/dataset/abnormal/*.wav`는 라벨 1입니다. 현장 원본 WAV를 학습·검증·테스트로 먼저 분리한 뒤 특징 윈도우를 생성하므로, 같은 파일의 윈도우가 서로 다른 분할에 섞이지 않습니다.
...
```bash
python -m audio_guard.interfaces.cli.train_cli --pipeline v5 --esc50-dir data/ESC-50 --field-data-dir data/dataset --field-splits field_splits.csv
python -m audio_guard.interfaces.cli.evaluate_cli --pipeline v5 --esc50-dir data/ESC-50 --field-data-dir data/dataset --field-splits field_splits.csv --min-field-recall 0.80
pytest -q tests/test_field_dataset.py tests/test_feature_windows.py tests/test_audio_v5.py
```
...
평가가 선택한 threshold와 모델 경로, 전처리 버전, 샘플레이트, 라벨 매핑을 `models/audio_model_v5.config.json`에 저장합니다. ESC-50과 현장 독립 테스트 결과를 모두 검토한 뒤, API(`main.py`)와 예측 CLI에서 이 JSON의 절대 경로를 `AUDIO_CONFIG_PATH`에 지정합니다.

기본 `configs/audio_config.json`은 기존 V3 모델과 threshold 0.7을 유지합니다. `GET /health`에서는 활성 모델, 전처리 버전, threshold를 확인할 수 있습니다. ESC-50 결과만 보고 V5를 배포하지 말고, 독립적으로 수집한 라즈베리파이 3초 현장 녹음도 테스트하세요.
